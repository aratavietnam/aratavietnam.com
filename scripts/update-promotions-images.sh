#!/bin/bash
# Script to update featured images for the 'promotion' post type.
...
process_post() {
    ...
    echo "Processing Promotion ID #$POST_ID: $POST_TITLE"

    KEYWORDS=$(echo "$POST_TITLE" | tr ' ' ',' | tr -d '[:punct:]')
    IMAGE_URL="${UNSPLASH_BASE_URL}?${KEYWORDS},sale,discount,shopping"
    ...
    echo " -> SUCCESS: Featured image updated for Promotion ID #$POST_ID."
    rm "$DOWNLOAD_PATH"
}
...
if [ -z "$POST_IDS" ]; then
    echo "No promotions of type '$POST_TYPE' are missing a featured image."
else
