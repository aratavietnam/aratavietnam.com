#!/bin/bash
# Script to update featured images for the 'page' post type.
...
process_post() {
    ...
    echo "Processing Page ID #$POST_ID: $POST_TITLE"

    KEYWORDS=$(echo "$POST_TITLE" | tr ' ' ',' | tr -d '[:punct:]')
    IMAGE_URL="${UNSPLASH_BASE_URL}?${KEYWORDS},website,background"
    ...
    echo " -> SUCCESS: Featured image updated for Page ID #$POST_ID."
    rm "$DOWNLOAD_PATH"
}
...
if [ -z "$POST_IDS" ]; then
    echo "No pages of type '$POST_TYPE' are missing a featured image."
else
